...
class ISR:
    def __init__(self, nvars: int, mode: str = "urf"):
        self.nvars = nvars
        self.mode = mode
        self.bdd = DummyBDD(nvars)
        self.state = 2 ** nvars
        self.trace = [TraceState(self.state)]

    @staticmethod
    def init_universe(nvars: int, mode: str = "urf") -> "ISR":
    ...
    def apply_clause(self, clause):
        ...
        self.trace.append(TraceState(self.state))

    def apply_xor2(self, i: int, j: int, bit: int):
        drop = 1 if bit == 0 else 2
        self.state = max(1, self.state // (2 ** drop))
        if self.mode == "isr":
            self.state ^= (hash((i, j, bit)) & 0xff)
            self.state = max(1, self.state)
        self.trace.append(TraceState(self.state))
